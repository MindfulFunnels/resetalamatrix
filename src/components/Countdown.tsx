...
const Countdown = ({ deadline }: { deadline: string }) => {
  const calculateTimeLeft = () => {
    const now = DateTime.now(); // Hora local del usuario
    const targetDate = DateTime.fromISO(deadline, { zone: "Europe/Madrid" }); // Hora base del evento en España
  ...
  return (
    <>
      ...
      <p>
        <span className='text-[#A85FE8]'>Hasta el evento</span>
      </p>
    </>
  );
};
